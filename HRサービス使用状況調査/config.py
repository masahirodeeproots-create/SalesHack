...
# HTTP共通定数（既存analyze_media_structure.pyのパターンを踏襲）
REQUEST_INTERVAL = 1.5  # 各リクエスト間のsleep（秒）
MAX_RETRIES = 3  # 502/503エラー時の最大リトライ回数
RETRY_BACKOFF_BASE = 2  # リトライ待機時間の底（秒）: 2^n秒ずつ増加
USER_AGENT = (
